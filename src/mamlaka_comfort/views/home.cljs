...
(defn hero []
  [:section.relative.bg-mf-gray.overflow-hidden
   [:div.container.mx-auto.px-4.py-20.flex.flex-col.md:flex-row.items-center
    [:div.md:w-1/2.z-10
     [:h1.text-5xl.md:text-7xl.font-black.text-mf-blue.leading-tight.mb-6
     ...
     [:div.flex.gap-4
      ...
      [:a.border-2.border-mf-blue.text-mf-blue.px-6.py-3.rounded.font-bold.hover:bg-mf-blue.hover:text-white.transition-all
       {:href "#contact"} "Visit Our Shop"]]]
    [:div.md:w-1/2.mt-12.md:mt-0
     [:img.rounded-lg.shadow-2xl.rotate-3
      {:src "https://images.unsplash.com/photo-1540518614846-7eded433c457?auto=format&fit=crop&q=80&w=800"
       :alt "Premium Bedding"}]]]
   [:div.absolute.bottom-0.right-0.bg-mf-red.text-white.px-8.py-4.font-bold.text-lg.hidden.md:block
